...
{% if correlated_risks %}
<h2>Correlated Risks</h2>
<p>Cross-agent analysis identified the following compound risks:</p>
{% for risk in correlated_risks %}
<div class="card">
  <h3>{{ risk.name }} <span class="badge {{ risk.severity }}">{{ risk.severity | upper }}</span></h3>
  <p>{{ risk.description }}</p>
  <p><strong>Agents involved:</strong> {{ risk.agents_involved | join(', ') }}</p>
  {% if risk.remediation %}<p><strong>Remediation:</strong> {{ risk.remediation }}</p>{% endif %}
</div>
{% endfor %}
{% endif %}
